...
public class IndexParseUtil {
     ...
     * @param list
     */
    public static void initMACD(List<StickModel> list) {
        if(list == null) return;
        //1计算出所有的DIF
     ...
     * @param list
     */
    public static void initKDJ(List<StickModel> list) {
        if(list == null) return;
        //1计算出所有的REV
        for(int i = 0; i < list.size(); i++) {
            if(i + START_REV <= list.size()) {
                //第9日开始计算RSV
                StickModel data = list.get(i + START_REV - 1);
        ...
        for(int i = 0; i < list.size(); i++) {
            if(i + START_DJ <= list.size()) {
                StickModel data = list.get(i + START_DJ - 1);
     ...
     * @param list k线数据
     */
    public static void initSma(List<StickModel> list) {
        if (list == null) return;
        for (int i = 0; i < list.size(); i++) {
            for (int j : SMA) {
                if (i + j <= list.size()) {
     ...
     * @return
     */
    private static double[] getMaxAndMin(List<StickModel> datas) {
        if(datas == null || datas.size() == 0)
            return new double[]{0, 0};
        double max = datas.get(0).getHigh();
        double min = datas.get(0).getLow();
        for(StickModel data : datas) {
     ...
     * @return
     */
    private static double getCountSma(List<StickModel> datas) {
     ...
     * @return
     */
    private static double getCloseSma(List<StickModel> datas) {
        if (datas == null) return -1;
        double sum = 0;
        for (StickModel data : datas) {
            sum += data.getClose();
     ...
     * @return
     */
    private static double getDifSma(List<StickModel> datas) {
        if (datas == null) return -1;
        double sum = 0;
        for (StickModel data : datas) {
            sum += data.getDif();
     ...
     * @return
     */
    private static double getRSVSma(List<StickModel> datas) {
        if (datas == null) return -1;
        double sum = 0;
        for (StickModel data : datas) {
            sum += data.getRsv();
     ...
     * @return
     */
    private static double getKSma(List<StickModel> datas) {
        if (datas == null) return -1;
        double sum = 0;
        for (StickModel data : datas) {
            sum += data.getK();
